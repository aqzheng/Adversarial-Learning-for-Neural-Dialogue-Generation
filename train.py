...
def disc_train_data(sess, gen_model, vocab, source_inputs, source_outputs,
    ...
    for query, answer in zip(source_inputs, source_outputs):
        query = query[:query_len] + [int(util.PAD_ID)] * (query_len - len(query) if query_len > len(query) else 0)
        train_query.append(query)
        answer = answer[:-1] # del tag EOS
        answer = answer[:answer_len] + [int(util.PAD_ID)] * (answer_len - len(answer) if answer_len > len(answer) else 0)
    ...
    def decoder(num_roll):
        for _ in xrange(num_roll):
            ...
            for i, output in enumerate(resps):
                output = output[:answer_len] + [util.PAD_ID] * (answer_len - len(output) if answer_len > len(output) else 0)
